...
<template>
  <div class="min-h-screen flex items-center justify-center bg-gray-100 relative">
    <!-- Image de fond -->
    <div
      class="absolute inset-0 bg-cover bg-center filter blur-sm brightness-75"
    ...
    <div class="relative bg-white p-8 rounded-3xl shadow-lg w-full max-w-2xl z-10">
      ...
      <div v-if="trips.length" class="space-y-4">
        <div
          v-for="trip in trips"
          :key="trip.id"
          class="p-4 bg-gray-200 rounded-lg shadow hover:shadow-lg transition"
        >
          ...
          class="flex items-center justify-center bg-green-500 text-white py-2 px-4 rounded-lg hover:bg-green-600 transition"
        >
